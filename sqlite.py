...
class database(object):
    def __init__(self, name:str=":memory:"):
        ...
        self.name = name
        self.master = self.get_table("sqlite_master")
    ...
    def create_table(self, name:str, columns:dict):
        ...
        self.do(f"CREATE TABLE IF NOT EXISTS {name} ({', '.join(l)})")
    ...
    def append_data_ifDNE(self, table:str, *values):
        """
        Insert a row of data to the table *if that data does not exist in the table*
        :param table: name of the table
        :param values: values you want to add. values should be given in ascending order. NO NEED for putting the values into a list / tuple first
        :return: None
        """
    ...
    def append_many_data(self, table:str, *values:list):
        ...
        :return: None
        """
        self.domany(f"INSERT INTO {table} VALUES ({', '.join(['?' for _ in values[0]])})", values)
    ...
    def get_table(self, name):
        ...
        class table(object):
            ...
            def append_ifDNE(self, *values):                                     return self.db.append_data_ifDNE(self.name, *values)
            ...
            def get_columns(self):                                               return self.db.get_table_columns(self.name)

        return table(self, name)

    def get_table_info(self, table_name):
        return self.do(f"PRAGMA table_info({table_name})").fetchall()

    def get_table_names(self, filter: str = None):
        return [i[0] for i in self.master.get(0, ("name",), filter)]
